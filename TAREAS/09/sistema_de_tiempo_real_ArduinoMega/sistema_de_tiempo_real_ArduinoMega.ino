...
TaskHandle_t TaskSerial_Handler;    //Añadimos manerjador de tareas
TaskHandle_t Tarea_Handler, Tarea7_Handler;  //Añadimos manerjador de tareas
TaskHandle_t xHandle;  //Añadimos manerjador de tareas
//---------------------
//-----[ AJUSTES ]-----
//---------------------
void setup() {
  ...
  delay(1000);
  mensaje = xQueueCreate(10, // Longitud de cola
...
void Tarea6( void * parameter ) {
  for ( int i = 0; i < 10; i++ ) { // Número de veces que ocurre esta tarea
    switch (i) {
      case 0:     // Primera ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 1:     // Segunda ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 2:     // Tercera ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 3:     // Cuarta ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 4:     // Quinta ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 5:     // Sexta ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 6:     // Séptima ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 7:     // Octava ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      case 8:     // Novena ejecución T6
        {
          String valueFromQueue = " ";
          Serial.print("------------- 06");   // Evidencia de ocurrencia
          if (xQueueReceive(mensaje, &valueFromQueue, portMAX_DELAY) == pdPASS) {
            Serial.println(valueFromQueue);    // Imprimimos lo que recibe de la otra función
          }
          // Serial.println("[<-1]");
          break;
        }
      case 9:     // Décima ejecución T6
        Serial.println("------------- 06");   // Evidencia de ocurrencia
        break;
      default:
        Serial.println("------------- 06");   // Evidencia de ocurrencia
  ...
  */
  ...
  for (;;) // Ciclo infinito
  {
    while (Serial.available() > 0) {
      switch (Serial.read()) {
        case 's':                         //en el caso de recibir s suspendemos la tarea 7 y 8
        Serial.println("Suspender 7 y 8");
          vTaskSuspend(Tarea7_Handler);
          vTaskSuspend(Tarea_Handler);
          break;
        case 'r':                       //en el caso de recibir r se reanudan las tareas 7 y 8
         Serial.println("Reanudar 7 y 8");
          vTaskResume(Tarea7_Handler);
          vTaskResume(Tarea_Handler);
         break;
         case '>':                      //en el caso de recibir > se aumenta la prioridad de la tarea 1
         Serial.println("Prioridad 01");
         vTaskPrioritySet(  xHandle, 3 );
          break;
          case 'x':                     //en el caso de recibir x se suspenden todas las tareas
